...
NAMED_CONTACT_PATTERN = re.compile(r"\(\s*gặp\s+(?:cô|thầy|ông|bà)\b", re.IGNORECASE)
PERSONAL_DATA_NOTICE = (
    "[Đã lược bỏ thông tin liên hệ cá nhân khỏi corpus. "
    "Sinh viên cần tra cứu kênh liên hệ chính thức hiện hành của đơn vị đào tạo.]"
)
...
def clean_extracted_content(content: str, title: str) -> str:
    """Remove common page chrome left behind around otherwise valid article text."""
    cut_positions = [position for marker in CONTENT_END_MARKERS if (position := content.find(marker)) >= 0]
    if cut_positions:
    ...
    notice_added = False
    for line in lines:
        if EMAIL_PATTERN.search(line) or NAMED_CONTACT_PATTERN.search(line):
            if not notice_added:
                cleaned_lines.append(PERSONAL_DATA_NOTICE)
                notice_added = True
            continue
...
def main() -> int:
    ...
    for index, row in enumerate(rows):
        ...
        try:
            ...
            content = clean_extracted_content(content, metadata["title"])
            if len(content) < 80:
